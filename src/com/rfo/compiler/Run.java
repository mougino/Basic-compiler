...
import com.rfo.compiler.Basic.TextStyle;
import com.rfo.compiler.GPS.GpsData;
...
//	Log.v(LOGTAG, "Line Buffer  " + ExecutingLineBuffer.line());
  ...
  // done at the end of the onCreate() method, and conditions starting the interpreter
    ...
    switch (requestCode) {
      case MANIFEST_PERMISSION_REQUEST:
        if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
          ...
          Toast.makeText(getContext(),"Permission denied by user! Quitting...", Toast.LENGTH_SHORT).show();
          finish();
        }
        break;
      default:
  ...
  public static String[] getPermissionsFromManifest(Context context) {
	...
	public enum FileType { FILE_TEXT, FILE_BYTE, FILE_ZIP };
    ...
    // same for zip file as input
		public static IOException closeStream(ZipFile zip, IOException ex) {	// close a zip
			if (zip == null) { return ex; }
			try { zip.close(); return ex; }
			catch ( IOException e ) { return (ex == null) ? e : ex; }
		}
	...
	public static class ZipWriterInfo extends FileInfo {
		...
		public ZipWriterInfo(int mode) { super(mode, false); }	// false means it is byte, not text
	...
	public static class ZipReaderInfo extends FileInfo {
		...
		public ZipReaderInfo(int mode) { super(mode, false); }	// false means it is byte, not text
	...
	private static final String BKW_JAVAC = "javac(";
  private static final String BKW_INSTALL_APK = "installapk(";
  private static final String BKW_DEX = "dex(";
	private static final String BKW_SIGNAPK = "signapk(";
	...
	public static final String BasicKeyWords[] = {
		...
		BKW_SELECT, BKW_TGET,
		BKW_FILE_GROUP, BKW_TEXT_GROUP, BKW_ZIP_GROUP, BKW_BYTE_GROUP, BKW_READ_GROUP,
...
//		Log.v(LOGTAG, "isOld  " + isOld);
    ...
    // Request permissions at RunTime on API > 23
		if (Build.VERSION.SDK_INT >= 23) {
			String[] manifestPermissions = getPermissionsFromManifest(getContext());
			boolean got_perms = true;
			for (String perm: manifestPermissions) {
				if ( PackageManager.PERMISSION_GRANTED != ContextCompat.checkSelfPermission(this, perm) ) {
					got_perms = false;  // at least 1 permission from manifest was not granted by user
				}
			}
			if (got_perms) {  // Permissions already granted --> starting activity ...
				mInterpreter = new Interpreter();		// start the interpreter in a background Thread
				mInterpreter.start();
			} else {          // At least 1 permission not already granted --> ask for it
...
//			Log.d(LOGTAG, "set DB_LOCK wait");
	...
	private final Command[] BASIC_cmd = new Command[] {
		...
		new Command(BKW_JAVAC)                  { public boolean run() { return executeJAVAC(); } },
		new Command(BKW_INSTALL_APK)            { public boolean run() { return executeInstallApk(); } },
		new Command(BKW_DEX)                    { public boolean run() { return executeDEX(); } },
		new Command(BKW_SIGNAPK)                { public boolean run() { return executeSignApk(); } },
...
//			if (op == RPRN) break;
	...
	private boolean executeSF_VERSIONX() {												// VERSIONX$
		if (!isNext(')'))				return false;	// Function must end with ')'
    try {
	...
	private boolean executeSF_SYSPATH() {												// SYSPATH$
		if (!isNext(')'))				return false;	// Function must end with ')'
		StringConstant = getContext().getFilesDir().getParent() + "/";
		return true;
	}
  ...
  // Custome OutputStream to catch JAVAC errors in order to put them in GetError$()
  ...
  private boolean executeInstallApk() {
    ...
    // Get path to apk
    if (!getStringArg()) return false;
    String apkpath = StringConstant;
		if (!isNext(')'))	return false;	// Function must end with ')'
		if (!isEOL()) 		return false;

    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      if (!getPackageManager().canRequestPackageInstalls()) {
        // We do not have permission to install from unknown sources --> ask for it
        startActivity(new Intent(android.provider.Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES));
      } else {
        // We have all needed permissions, launch install
        File apk = new File(Basic.getDataPath(apkpath));
        if (apk.exists()) {
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Uri apkUri = GenericFileProvider.getUriForFile(getContext(), "com.rfo.compiler.provider", apk);
            Intent intent = new Intent(Intent.ACTION_INSTALL_PACKAGE);
            intent.setData(apkUri);
            intent.setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            startActivity(intent);
          } else {
            Uri apkUri = Uri.fromFile(apk); // Create Uri for the file
          ...
          writeErrorMsg("APK does not exist");
        }
      }
    }
		return true;
  ...
  // DEX: dexify Java classes into 'classes.dex'
  private boolean executeDEX() {
    clearErrorMsg();

    // Get path to bin/ folder containing all the classes
    if (!getStringArg()) return false;
    String classpath = StringConstant;
		if (!isNext(')'))	return false;	// Function must end with ')'
		if (!isEOL()) 		return false;

    String[] dexoptions = new String[] {
        "--dex",
        "--output=" + Basic.getDataPath("classes.dex"),
        classpath
    };
    
    Log.d(LOGTAG, "Dexifying " + classpath);

    // Redirect System.err to our custom OutputStream
    System.setErr(new java.io.PrintStream(errorOutputStream));

    // Call Dx
    com.android.dx.command.Main.main( dexoptions );
	...
	// Kellinwood's ZipSigner
  private boolean executeSignApk() {
    clearErrorMsg();

		// Get path to (unsigned) input APK
    if (!getStringArg()) return false;
    String unsigned = StringConstant;
		if (!isNext(')'))	return false;	// Function must end with ')'
		if (!isEOL()) 		return false;

    // Set path to private certificate and public key
    String keypath = getContext().getFilesDir().getParent(); // path '/data/data/<rfo-compiler>'

    // Set path to final apk
    String finalApk = unsigned.replace("-unsigned", "");
    if (finalApk == unsigned) finalApk = unsigned.replace(".apk", ".signed.apk");

    Log.d(LOGTAG, "Signing " + unsigned);

    try {
      // Create a ZipSigner object
      ZipSigner zipSigner = new ZipSigner();

      // Load X509 certificate
      URL pubkUrl = new URL("file://" + keypath + "/cert.x509.pem");
      X509Certificate pubk = zipSigner.readPublicKey(pubkUrl);

      // Load PKCS8 key
      URL privkUrl = new URL("file://" + keypath + "/key.pk8");
      PrivateKey privk = zipSigner.readPrivateKey(privkUrl, null /* no pwd b/c key not encrypted */);

      // Sign our apk with these custom certificate/key
      zipSigner.setKeys("custom", pubk, privk, null);
      zipSigner.signZip(unsigned, finalApk);
    }
    catch (Throwable t) {
      writeErrorMsg(t.toString());
    }
    ...
    ZipFile reader = null;
      ...
      reader = new ZipFile(new File(Basic.getDataPath(fileName)));
	...
	private boolean executeZIP_OPEN() {			// Open a file
		...
		if (FileMode == FMR) {										// Read was selected
			...
			if (buf != null) {
				fInfo.mZipReader = buf;							  // store reader in fInfo
			} else {
				fileNumber = -1;									    // change file index to report file does not exist
			}
			FileTable.add(fInfo);
			...
			if (file.exists() && file.canWrite()) {   // open the zip writer for the SD Card
				try { writer = new ZipOutputStream(
			...
			if (writer != null) {
				fInfo.mZipWriter = writer;			// store writer in fInfo
			} else {
				fileNumber = -1;								// change file index to report file does not exist
			}
			FileTable.add(fInfo);
    ...
    ZipFile reader = ((ZipReaderInfo)fInfo).mZipReader;

    try {
      ZipEntry entry = reader.getEntry(fileName);
      if (entry == null) {                               // entry not found in the zip: return "EOF"
        val.val("EOF");
      } else if (entry.isDirectory()) {
        return RunTimeError("Zip entry must be a file:");
      } else {
        InputStream zis = reader.getInputStream(entry);
    ...
    if (!getStringArg())  return false;     // get the entry path
    String fileName = StringConstant;
    if (isNext(',')) {                      // optional 'uncompressed' flag: if set to
      if (!evalNumericExpression()) {       // non-zero, file will be zipped uncompressed
        return false;
      }
			uncompressed = EvalNumericExpressionValue.intValue();
    }
		if (!checkEOL())			return false;
		...
		ZipOutputStream writer =  ((ZipWriterInfo)fInfo).mZipWriter; // get the writer to the zip
    if (uncompressed > 0) {   													 // explicitly asked uncompressed
      writer.setMethod(java.util.zip.ZipEntry.STORED);
    } else {
      writer.setMethod(java.util.zip.ZipEntry.DEFLATED);
			writer.setLevel(Deflater.DEFAULT_COMPRESSION);
    }
    ...
    try {
      ...
      ZipEntry entry = new ZipEntry(fileName);
			...
			if (uncompressed > 0) {   					// explicitly asked uncompressed
				...
				Log.i(LOGTAG, "ZipWrite:Uncompressed:length:" + entry.getSize());
				final CRC32 checksum = new CRC32();
				byteArray = new byte[is.available()];
			  is.read(byteArray);
				checksum.update(byteArray);
				Log.i(LOGTAG, "ZipWrite:Uncompressed:checksum:" + checksum.getValue());
				entry.setCrc(checksum.getValue());
			}
      writer.putNextEntry(entry);                   // create entry in the zip
			if (uncompressed > 0) {
				writer.write(byteArray, 0, byteArray.length);	// write data to it
			} else {
				while ((read = is.read(data, 0, 1024)) != -1) {
					writer.write(data, 0, read);                // stream data to it
				}
			}
    }
    catch (IOException e) { return RunTimeError("I/O error at:"); }
